...
test("sql console - displays and runs queries", async ({ page }) => {
  ...
  // Run a simple query (the example queries may not work with test fixture)
  const textarea = page.getByPlaceholder("SELECT * FROM cards");
  await textarea.fill("SELECT id, nid, did FROM cards LIMIT 5");
  await page.getByRole("button", { name: "Run" }).click();

  // Verify results appear
  await expect(page.getByText(/\d+ rows in \d+ms/)).toBeVisible();
...
test("sql console - example query buttons", async ({ page }) => {
  ...
  await expect(textarea).toContainText("revlog");
});

test("sql console - error handling for invalid query", async ({ page }) => {
  await page.goto("/?view=sql");

  // Try to run a DROP query (should be blocked)
  const textarea = page.getByPlaceholder("SELECT * FROM cards");
  await textarea.fill("DROP TABLE cards");
  await page.getByRole("button", { name: "Run" }).click();

  // Should show error message
  await expect(page.getByText(/Only SELECT queries are allowed/)).toBeVisible();

  // Try INSERT (should be blocked)
  await textarea.fill("SELECT * FROM cards; INSERT INTO cards VALUES (1)");
  await page.getByRole("button", { name: "Run" }).click();
  await expect(page.getByText(/forbidden keyword/i)).toBeVisible();
});

test("sql console - custom query", async ({ page }) => {
  await page.goto("/?view=sql");

  // Run a custom simple query
  const textarea = page.getByPlaceholder("SELECT * FROM cards");
  await textarea.fill("SELECT COUNT(*) as total FROM cards");
  await page.getByRole("button", { name: "Run" }).click();

  // Verify results - 1 row returned with count
  await expect(page.getByText("1 rows")).toBeVisible();
  await expect(page.getByRole("columnheader", { name: "total" })).toBeVisible();
  // Verify we got a count (fixture has cards)
